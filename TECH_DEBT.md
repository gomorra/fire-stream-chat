...
**Fix sketch (cheap path).** Make the send-side block check non-fatal offline: catch the Firestore offline / `UNAVAILABLE` exception in `isUserBlocked` (or its callers) and treat an undeterminable result as "not blocked", letting the send proceed into the normal pipeline — where offline is already handled correctly (text queues via Firestore's write buffer; media's upload fails *inside* `failSendOnError` → `FAILED` + retry). Sender-side block enforcement degrading open while offline is low-risk: the recipient still filters blocked senders on receive, and server rules still apply. Alternatively, move `messageDao.insertMessage(...)` ahead of the block check so even a thrown check leaves a `SENDING` row that orphan-recovery flips to `FAILED` — but fail-open is cleaner and matches the existing receive-side behavior.

**When to revisit.** Fold into the next offline-send work (the Durable offline-send outbox above), or fix standalone the next time block-related code is touched — a ~10-line change plus a repository test that stubs `isUserBlocked` to throw a `FirebaseFirestoreException` and asserts the optimistic row is still inserted (and lands in `FAILED` for media).
...
**The smell.** Images have no thumbnail. `mediaThumbnailUrl` is generated and stored for **VIDEO only** (`MessageRepositoryImpl` send pipeline, `isVideo` branch); the image branch leaves it null, and `MediaAttachment.thumbnailUri` is defined but unused. So the Shared Media grid and every image bubble decode the full-resolution `mediaUrl` (or on-disk full file) and rely on downsampling at decode time. The July 2026 black-tile fix made the grid decode safe (it routes tiles through Android `ImageDecoder` via `ScaledImageDecoder`, avoiding the `BitmapFactory` heavy-subsample path that returned black bitmaps, plus a local-file preference) but did not eliminate the underlying waste: every gallery tile still fetches/decodes a full-size image.
