...
- **`/code-review`** — correctness bugs, which `/simplify` explicitly does not hunt. **Mandatory, not a judgment call**, on any diff touching Signal/crypto, coroutine scoping, the sync/outbox path, or the build files (`app/build.gradle.kts`, `build.gradle.kts`, `settings.gradle.kts`, `gradle/libs.versions.toml`) — a `git add -A` once swept an unrelated `compileSdk` and hard-coded `versionCode` change into a bug-fix diff and shipped a release APK that would not install, with the test gate green throughout. Also mandatory where a plan's `skills:` tag names it, and before cutting a release. Judgment-gated in post-step item 4 everywhere else. `/code-review ultra` is user-triggered and billed — Claude cannot launch it, so never write it into an auto-run step.
