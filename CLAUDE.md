...
3. `/simplify` — code review of the pending diff. **Before invoking, state which of triggers (a)–(d) fire, or "none."** Then pick the path:
   - **Default (no trigger fires)** — sub-agent wrapper at Sonnet/medium: `Agent(subagent_type: "general-purpose", model: "sonnet", prompt: "<diff context> — invoke Skill(skill: 'simplify')")`. The sub-agent must **not** reimplement the review with a custom prompt. Note: sub-agents lack the `Agent` tool, so the skill's Phase 2 (three parallel reviewers) degrades to a single inline pass. Acceptable cost for low-risk diffs; buys you context isolation.
   - **Any trigger fires** — invoke `Skill(skill: "simplify")` **directly in the main conversation** (no sub-agent wrapper). Main-context Claude has the `Agent` tool, so the skill's Phase 2 actually runs three parallel review agents with independent contexts. That's the entire point of escalation; wrapping it in a sub-agent gives you the token cost of Opus without the parallelism. Accept the context pollution as the cost of the review you actually want.
   - **Never substitute `/simplify-review`** — that skill is manual-invocation only.
   - **Triggers:** (a) concurrency-/state-machine-heavy (coroutine scoping, flow operator chains, cancellation, lock ordering); (b) security-adjacent (Signal/crypto, permission checks, auth); (c) cross-cutting across many layers (DI + repo + multiple ViewModels + workers in one change); (d) large (>~600 changed lines). Prefer bumping to main-context path over bumping effort.
