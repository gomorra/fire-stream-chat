...
| `app/src/main/java/com/firestream/chat/ui/chat/ImagePreviewScreen.kt` | Pager over the picked batch — per-item caption, thumbnail strip, remove-before-send, editor rail; a page's pinch-zoom is the crop that is sent, flattened on Send and before an editor opens |
...
| `app/src/test/java/com/firestream/chat/ui/chat/imageedit/ViewportGeometryTest.kt` | Zoom ⇄ frame arithmetic on the JVM — the visible crop at each zoom and pan, the clamp on both axes, and the restore round-trip into a box of the same and another shape |
| `app/src/test/java/com/firestream/chat/ui/chat/ImagePreviewScreenZoomTest.kt` | A zoom is the crop that is sent — through Coil on a real JPEG: the flatten on Send and before an editor opens, the failure line, and the frame surviving a recreation |
