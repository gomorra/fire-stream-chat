...
**Definition.** Each editor screen takes a source `Uri`, and on **Done** flattens its layer into a new full-size JPEG in `cacheDir/edits/` via `ImageEditRasterizer.rasterize`, handing that URI back. `PendingMedia` points at the newest file and keeps `originalUri` untouched, so the chain of files *is* the undo history — `editCursor` moves, the files stay, and the send pipeline receives a different URI without ever learning that editing exists. Everything an overlay tool positions (a stroke, a sticker, a text run) is stored **normalized to the image being edited**, `0..1`, and converted at the boundary through `ImageFitMapper`. **A pinch-zoom on a fullscreen photo is a crop, not a view:** `ZoomCropSurface` keeps a page's zoom, plus the shape the crop pill chose, as a `PendingCrop` normalized to the image (`ViewportGeometry`, `CropGeometry.fitInside`). `ImagePreviewScreen` flattens it through this same `rasterize` → `landEdit` path the moment it has to become pixels — on Send, and before Draw or Overlay open on that page — so it is a history step like any other and the page then shows the cropped step at 1x, which is the picture the zoom was showing; Adjust has a crop tool, so it gets the frame *transferred* (`initialCrop` / `initialAspect`) and its Done writes it. The fullscreen viewer keeps the same `PendingCrop` for the page on screen and hands it to `editFromViewer`, which carries it into the preview as `PendingMedia.initialCrop`.

**Use when.** Adding any editor screen or tool under `ui/chat/imageedit/`. Convert Compose state into `domain.util.RasterOp` at the boundary — the ops carry plain floats and `Long` ARGB colours, never Compose types. `RasterOp` and the dimension arithmetic live in `domain/util/ImageEditGeometry.kt` precisely so a screen can build and label its ops without importing `data/`; `ImageEditRasterizer` is the platform half and stays the editor's single `UI_ALLOWED_DATA_IMPORTS` entry, needed only by the hosting ViewModel.
...
**Example.** `app/src/main/java/com/firestream/chat/domain/util/ImageEditGeometry.kt` (the ops and the arithmetic) and `data/util/ImageEditRasterizer.kt` (decode, encode, the 4096 px ceiling in practice, the cache lifecycle); `ui/chat/PendingMedia.kt:85` `landEdit` (append, truncate the abandoned tail, hand those files back for deletion) and `:115` `onSurvivingStep` (fall back when the OS reclaimed one); `ui/chat/imageedit/ImageFitMapper.kt`. Full reasoning: `docs/plans/image-editor.md` §2.1, §2.3, §2.7.

**Trap.** Three, all of which look like tidiness until they bite.
- **Forgetting the abandoned files.** Redo means undo can no longer free the file it steps off, so the cache grows per edit *step* and stays grown for the whole preview session. `landEdit` returns them; hand them to `discard` or leak until the next app start. The edits live in `ImagePreviewScreen`'s own `drafts` and are never lifted back to the caller's pick list, so a dismiss path that reads the caller's list discards nothing and looks like it worked.
...
- **Applying the preview's zoom anywhere but through the history.** The send pipeline never learns that editing exists (above), so the zoom-crop is flattened into a step *before* `onSend` is called, never carried on `PendingMedia` for the compressor to apply. And it is normalized to the photo's *displayed* size (Coil's painter, orientation applied), not to the header-only `probeSource`, which does not read EXIF orientation — a frame normalized to the unrotated header would crop the wrong part of every camera photo.
- **Letting the preview and the flatten each compute their own numbers.** A layer is rendered *twice* — by Compose while the user is deciding, by `android.graphics` when Done writes the file — and the two graphics stacks share no path, paint or bitmap object. Everything that decides *what* is drawn (a stroke's width against the long edge, the curve through its samples, how coarse a mosaic is) therefore goes in `domain/util/` and is called by both; only the painting itself is written twice. `StrokeGeometry` is the worked example. On a pen mark a divergence is cosmetic; on a blur it is a redaction that covered the face on screen and missed it in the JPEG.
