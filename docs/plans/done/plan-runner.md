...
| **The `mid` tier is Opus at `medium`, no longer Sonnet at `high`** (§0 said "today Sonnet"). Interim: the benchmark picks between its two variants and may replace it. | On Anthropic's SWE-bench Pro subset Opus at `low` (84.0% at $0.25 per solved task) dominates Sonnet at its default (77.4% at $0.84), and the two real Sonnet/high steps cost ~$2 each for trivial work. The benchmark decides between a and b — it never reopened whether Sonnet stays, so leaving it as the default meant a run without `--variant` used a configuration nobody believed in. | `run-plan.sh` tunables |
...
| **2026-09-20 review pass.** Validation has a sixth check: a `done` step leaves nothing uncommitted or untracked besides what it committed (checked before the gate re-run; nudge-able). A session that ends without its result object a second time, nudge spent, is escalated like an invalid one instead of blocked. A missing `git`/`jq`/`claude` is exit 1, not a "driver bug" or a blocked step. The plan-differs warning compares the main tree with the merge-base, so it fires on an edit the runner will not see, not on every run after the first `**Shipped**` block. The result's `step` is a string (`"5a"` is a legal id). | An external review of the driver; none of it had fired in a real run. The clean-tree gap was the one hole in "validate, trusting nothing": leftovers were noticed only at the next launch and handed to that session. | `run-plan.sh` `validate_step`, `handle_result`; `step-result.schema.json`; `e2e.sh` |
