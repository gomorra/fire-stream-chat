...
**Order: 1 → 2 → 3 → 4 → 5** (fully sequential, no checkpoint. Until 2026-09-20 a `‖` stopped the run after the minSdk step so the human could look at the fallout first; removed for the benchmark in `scripts/plan-runner/benchmark.md`, which measures how far a configuration gets unattended. Nothing is merged or pushed by the runner, so step 1 is reviewed at the end with the rest, with the judge's grade and the test-count delta in hand.)
...
- **Overruled by the user on 2026-09-20: the release is 1.35.0, a minor bump as §3 recommended** — a minSdk raise is not an API break for this app, and 2.0.0 is too large a jump for it. What the step decided at the time:
