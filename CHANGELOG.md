...
- **Emoji built from several characters no longer come apart.** Picking the heart on fire from the emoji panel put a heart and a flame in the composer instead of the single glyph, and the message went out looking the same way. Any emoji written as more than one character was affected — the family faces, a thumbs-up with a skin tone, a flag, a keycap — because the app sized each character on its own, and splitting the styling of such a sequence is what splits the glyph. Each one is now styled as a whole, so it renders as the one emoji it is, at one size.
...
- **The ticks on a sent message no longer flick back from read to delivered.** Once the other person had read a message, its orange ticks could briefly turn white and then orange again. The notification that announces a message to the recipient often arrives a few seconds after their open chat has already read it, and its delivery mark was overwriting the read mark on both sides, which made the chat read the message a second time. A delivery mark now only ever moves a message forward: a message that is already read stays read, and the recipient's delivery time is still recorded. (`69fcfd89`)
...
- **The app rebuilds its local copy of your messages once, on this update.** Messages gained the bookkeeping a retry needs to resume without encrypting a message twice, and then the bookkeeping a queued message needs, and the app resets its local message store whenever that layout changes. Every chat reloads its full history from the server the first time you open it, and photos, videos, documents and voice notes all stay available — files already on the phone are picked up without downloading again. What does not come back: messages that were still unsent or marked failed, starred marks, and scheduled message reminders. The encryption side changes nothing you can see yet, because end-to-end encryption is not switched on: once it is, several messages sent to one person at once can no longer garble each other, and a retried message goes out as the same encrypted copy. (`bcd4426c`, `2f8bc711`, `f1b5d887`)
