...
- **Undo and redo work in the send preview, one editor visit at a time.** The photo editor keeps every step it writes rather than replacing the last one, so the preview's undo, redo and original⇄edited controls are now live: you can walk back past a crop to a drawing three screens earlier, walk forward again, or flick to the untouched pick and back to exactly the step you were on. History is per photo, so it acts on the page you are looking at and never on the batch. Starting a new edit after undoing discards the steps you had walked back past, which is what keeps the editor's disk use bounded. (``)
- **Edited photos are worked on at up to 4096 pixels.** Editing a very large camera original at its true resolution would exhaust memory, so an edit pass caps the long edge — meaning an HD send of a photo you edited carries a little less detail than an HD send of the same photo untouched. A photo you never edit is unaffected. (``)

### Changed

- **Photo quality estimates now come from the same code that does the compressing.** The Standard and HD sizes in the quality sheet were worked out by a separate copy of the compressor's rules, which would have drifted silently the first time those rules changed. Both rows are now measured against the real thing, and the sheet leaves out a size it genuinely cannot work out rather than printing a number to fill the space. (``)
