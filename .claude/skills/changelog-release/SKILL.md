...
**Before appending, check the section has not shipped:** `scripts/check-changelog-header.sh` fails when the `[UNRELEASED] [X.Y.Z]` version already exists as tag `vX.Y.Z` (a tag made without `cut-release.sh` leaves the prefix on). Then drop the prefix from that header and open a new section above it — never upgrade a shipped section in place. CI runs the same check on every push.
